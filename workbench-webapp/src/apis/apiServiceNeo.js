...
export const fetchTokens = async () => {
  ...
  console.log(`Calling fetchToken at ${currentTime}`);

  try {
    const getSourceToken = await fetch(
      `${baseURL}/v1/migration/configuration/connect/source/is`,
      {
        method: "POST",
        body: JSON.stringify( currNeoAgent.NeoData ),
        headers: headers,
      }
    );
    if (!getSourceToken.ok) {
      throw new Error("Source token not fetched: " + getSourceToken.statusText);
    }

    const getTargetToken = await fetch(
      `${baseURL}/v1/migration/configuration/connect/target/is`,
      {
        method: "POST",
        body: JSON.stringify(currNeoAgent.CFdata ),
        headers: headers,
      }
    );
    if (!getTargetToken.ok) {
      throw new Error("Target token not fetched: " + getTargetToken.statusText);
    }
  } catch (error) {
    console.error("Error fetching Source or Target token:", error);
  } finally {
    // Schedule the next call
    setTimeout(fetchTokens, 30 * 60 * 1000); // 30 minutes
  }
};
...
export const readinessCheck = async () => {
  try {
    ...
    const data = await response.json() ;// Handle empty response body
    const parsedData = JSON.parse(data.payload);
    console.log("Readiness Check Data:", parsedData);
    ...
    let packageIds = [];
    let customPackageIds = [];
    let custPkgNotMigrate = new Set();
    ...
    for (let result of parsedData.d.results) {
      packageIds.push(result.Id);
      if (result.Vendor === "SAP" || result.PartnerContent === true) {
        totalPackages++;
        ...
        customPackages++;
        customPackageIds.push(result.Id);
        if (result.UpdateAvailable === true) {
          custPkgNotMigrated++;
        }
      }
    ...
    for (let id of packageIds) {
      ...
      const parsedVersionRes = JSON.parse(versionRes.payload)
      console.log(`Iflow Version response of ${id} -- `, versionRes);

      let packageIflows = parsedVersionRes.d.results.length;
      let packageIflowsNotMigrate = 0;
      let packageIflowsMigrate = 0;

      for (let result of parsedVersionRes.d.results) {
        if (result.Version === "Active" || result.Version === "Draft") {
          packageIflowsNotMigrate++;
          addToPkgNotMigrate(result.PackageId);
          if(customPackageIds.includes(result.PackageId)){
            custPkgNotMigrate.add(result.PackageId);
          }
        }
      }
      ...
      // Setting custom package not migrateable by the size of set.
      custPkgNotMigrated = custPkgNotMigrate.size;
      ...
      const parsedValueMappingRes = JSON.parse(valueMappingsRes.payload);
      console.log(`vm response of Package: ${id}: `, valueMappingsRes);

      // ----- Only for debugging, remove later
      if (parsedValueMappingRes.d.results.length > 0) {
        console.log(` ------------- Found Value Mapping in PACKAGE: ${id}`);
      }
      // -----

      totalValueMappings += parsedValueMappingRes.d.results.length;

      const draftArtifactNames = parsedValueMappingRes.d.results
        .filter(
    ...
    const parsedSourceJmsData = JSON.parse(sourceJmsData.payload);
    console.log("Source JMS Resources Data:", sourceJmsData);

    if (parsedSourceJmsData.error) {
      console.log(`Error - ${parsedSourceJmsData.error.message.value}`);
      // throw new Error(`${sourceJmsData.error.message.value}`);
    } else {
      const srcQueueCount = parsedSourceJmsData.d.QueueNumber || 0;
      ...
      const parsedTargetJmsData = JSON.parse(targetJmsData.payload);
      console.log("Target JMS Resources Data:", targetJmsData);

      const tgtMaxQueueCount = parsedTargetJmsData.d.MaxQueueNumber || 0;
...
export const GetPackages = async () => {
  ...
  try {
    ...
    const tokenParsed = await fetchToken.json()
    ...
    if (fetchToken && fetchToken.ok === true) {
      ...
      if (!fetchPackagesRaw.ok) {
        throw new Error(`Error: ${fetchPackagesRaw.statusText}`);
      }
      const fetchPackages = await fetchPackagesRaw.json();
      const parsedFetchPackages = JSON.parse(fetchPackages.payload)
      console.log(fetchPackages);

      for (let result of parsedFetchPackages.d.results) {
        if (result.Vendor === "SAP" || result.PartnerContent === true)
         {
      ...
      if (selectedPrePackages.length !== 0) {
        for (let pkg of selectedPrePackages) {
          ...
          try {
            ...
            console.log("postPrePackageRes - ",postPrePackage);

            if (postPrePackage.status !== 'Success') {
              setNotification({
                open: true,
                message: postPrePackage.message,
                severity: "error",
              });
              throw new Error(
                `Failed to create Pre-Package ${pkg.value} message: ${postPrePackage.message}`
              );
            } else {
      ...
      if (selectedCustomPackages.length !== 0) {
        for (let pkg of selectedCustomPackages) {
          ...
          try {
            ...
            if (postCustomPackage.status !== 'Success') {
              setNotification({
                open: true,
                message: postCustomPackage.message,
                severity: "error",
              });
              throw new Error(
                `Failed to create Custom-Package ${pkg.value} message: ${postCustomPackage.message}`
              );
            } else {
              console.log(
...
export const GetArtifacts = async (pkgId) => {
  ...
  try {
    ...
    // Not converting to json as in response for this call payload is array instead of string
    // const parsedGetArtifacts = JSON.parse(getArtifacts.payload)
    console.log("Iflow's fetched: ", getArtifacts);

    let artifactConfigured = 0;
    let artifactNotConfigured = 0;
    for (let artifact of getArtifacts.payload) {
...
export const configureValueMappings = async (pkgId) => {
  ...
  try {
    ...
    const parsedValueMappings = JSON.parse(valueMappings.payload);
    console.log("Value Mapping's fetched: ", valueMappings);

    for (let valueMapping of parsedValueMappings.d.results) {
...
export const fetchCertificates = async () => {
  try {
    ...
    const data = await response.json();
    const parsedData = JSON.parse(data.payload)
    const certificates = parsedData.map((cert) => ({
      id: cert.Hexalias,
      label: cert.Alias,
    }));
    return certificates;
    ...
    for (let certificate of selectedCertificates) {
      try {
        ...
        if (certResponse.ok) {
          setNotification({
            open: true,
            message: `Certificate ${certificate.label} uploaded successfully`,
            severity: "success",
          });
        } else {
          setNotification({
...
export const checkUploadedCredentials = async (
  ...
  try {
    ...
    if (!oauthResponse.ok) {
      throw new Error(oauthData.message || "Failed to fetch uploaded credentials");
    }
    setUploadedCredentials(oauthData.payload);
    ...
    if (!userResponse.ok) {
      throw new Error(userData.message || "Failed to fetch user credentials");
    }
    setuserCredentials(userData.payload);

    // If both requests are successful, show the modal
    setModalOpen(true);
...
export const fetchNumberRanges = async (setNumberRanges) => {
  try {
    ...
    const data = await response.json();
    const parsedData = JSON.parse(data.payload);
    setNumberRanges(parsedData.d.results);
  } catch (error) {
...
export const fetchVariables = async (setVariables) => {
  try {
    ...
    const data = await response.json();
    const parsedData = JSON.parse(data.payload);
    setVariables(parsedData.d.results);
  } catch (error) {
...
export const postVariables = async (selectedVariables, setNotification) => {
  ...
  const sourceToken = await fetchSourceToken.json();
    ...
    if (fetchSourceToken && fetchSourceToken.ok === true) {
      for (let selectedVariable of selectedVariables) {
        try {
          ...
          // Checking if the name of Variable is in CF Tenant
          ...
          const parsedVariablesCf = JSON.parse(variablesCf.payload)
          const variablesInCf = parsedVariablesCf.d.results.map(
      ...
      console.error("Source Token fetching not successful");
    }
};
...
export const fetchDataStores = async (setDataStores) => {
  try {
    ...
    const data = await response.json();
    const parsedData = JSON.parse(data.payload)
    setDataStores(parsedData.d.results);
  } catch (error) {
  ...
  console.log(sourceToken);
    ...
    if (fetchSourceToken && fetchSourceToken.ok === true) {
      for (let selectedDataStore of selectedDataStores) {
        try {
          const getDataStoreId = await fetch(
          ...
          const getDataStoreIdRes = await getDataStoreId.json();
          const parsedDataStoreId = JSON.parse(getDataStoreIdRes.payload);
          const dataStoreId = parsedDataStoreId.d.results[0].Id;
          console.log(
          ...
          const parsedDataStoresCf = JSON.parse(dataStoresCf.payload)
          const dataStoreInCf = parsedDataStoresCf.d.results.map(
