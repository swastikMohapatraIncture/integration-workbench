import { useState, useEffect } from 'react';
import PropTypes from 'prop-types';
import Tabs from '@mui/material/Tabs';
import Tab from '@mui/material/Tab';
import Typography from '@mui/material/Typography';
import Box from '@mui/material/Box';
import MigrateIP from '../components/MigrateIP';
import MigrateSA from '../components/MigrateSA';
import MigrateOA from '../components/MigrateOA';
import GetArtifacts from '../components/GetArtifacts';

function CustomTabPanel(props) {
  const { children, value, index, ...other } = props;

  return (
    <div
      role="tabpanel"
      hidden={value !== index}
      id={`simple-tabpanel-${index}`}
      aria-labelledby={`simple-tab-${index}`}
      {...other}
    >
      {value === index && (
        <Box sx={{ p: 0 }}>
          <Typography>{children}</Typography>
        </Box>
      )}
    </div>
  );
}

CustomTabPanel.propTypes = {
  children: PropTypes.node,
  index: PropTypes.number.isRequired,
  value: PropTypes.number.isRequired,
};

function a11yProps(index) {
  return {
    id: `simple-tab-${index}`,
    'aria-controls': `simple-tabpanel-${index}`,
  };
}

const GetArtifactsPage = () => {
  const [value, setValue] = useState(0);
  const [prepackages, setPrePackages] = useState([]);
  const [custompackages, setCustomPackages] = useState([]);

  useEffect(() => {
    const fetchPackages = async () => {
      const packages= await GetPackages();
      setPrePackages(packages.prepackages);
      setCustomPackages(packages.custompackages);
    };

    fetchPackages();
  }, []);

  const handleChange = (event, newValue) => {
    setValue(newValue);
  };

  return (
    <Box sx={{ width: '100%' }}>
      <Box sx={{ borderBottom: 1, borderColor: 'divider' }}>
        <Tabs value={value} onChange={handleChange} aria-label="basic tabs example">
          <Tab sx={{ textTransform: "capitalize" }} label="Package > Artifacts" {...a11yProps(0)} />
        </Tabs>
      </Box>
      <CustomTabPanel value={value} index={0}>
        <GetArtifacts prepackages={prepackages} custompackages={custompackages}  />
      </CustomTabPanel>
    </Box>
  );
};

export default GetArtifactsPage;
